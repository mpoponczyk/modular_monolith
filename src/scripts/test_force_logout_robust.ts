...
async function main() {
    console.log("--- STARTING ROBUST FORCE LOGOUT EXPERIMENT ---");
    ...
    console.log("2. Logging in as temp user to acquire Access Token...");
    ...
    console.log("3. Deleting auth.sessions row for this user...");
    // Using admin.signOut to rigorously delete the session representation
    const { error: delErr } = await adminClient.auth.admin.signOut(userId, 'global');

    if (delErr) {
        console.error("Failed to delete sessions:", delErr);
    } else {
        console.log("-> Success. Sessions revoked (deleted).");
    }
