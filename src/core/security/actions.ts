...
export async function requestChallengeAction(tenantSlug: string, locale: string = 'en') {
...
export async function verifyChallengeAction(tenantSlug: string, code: string) {
    const auth = await resolveAuthContext(tenantSlug);
    if (!auth) redirect('/login');
